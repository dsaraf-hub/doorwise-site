...
const riderLabel=document.createElement("div");riderLabel.className="rider-label";riderLabel.innerHTML="<i></i><span>Rider location</span>";host.appendChild(riderLabel);const scene=new THREE.Scene();const camera=new THREE.OrthographicCamera(-18,18,22,-22,.1,250);camera.position.set(37,29,40);
const controls=new OrbitControls(camera,renderer.domElement);controls.target.set(0,12,0);controls.enableDamping=true;controls.enablePan=false;controls.enableZoom=false;controls.minPolarAngle=.4;controls.maxPolarAngle=1.5;controls.autoRotate=true;controls.autoRotateSpeed=.24;controls.minAzimuthAngle=-.45;controls.maxAzimuthAngle=1.15;
...
const stageButtons=[...document.querySelectorAll('[data-stage]')];const labels=['Rider entered the building','Elevator journey in progress','Approaching the delivery door'];let progress=0,start=performance.now(),manualAt=-1,paused=matchMedia('(prefers-reduced-motion: reduce)').matches,lastStage=-1,visible=true;controls.autoRotate=!paused;
const reduce=matchMedia('(prefers-reduced-motion: reduce)');reduce.addEventListener('change',e=>{paused=e.matches;controls.autoRotate=!paused;updatePause()});
...
pauseBtn.onclick=()=>{paused=!paused;controls.autoRotate=!paused;start=performance.now()-progress*18000;updatePause()};document.querySelector('#reset').onclick=()=>{camera.position.set(37,29,40);controls.target.set(0,12,0);controls.update();progress=0;start=performance.now();manualAt=-1};
...
controls.addEventListener('start',()=>controls.autoRotate=false);controls.addEventListener('end',()=>{controls.autoRotate=!paused});
